...
class RemehaHomeAdapter extends utils.Adapter {
  ...
  async createDevices() {
    const scheduling = await (0, import_tools._translate)("Scheduling", systemLang);
    ...
    const states = [
      ...
      { id: "data.dhw.dhwTemperature", name: "DHW Temperature", read: true, write: false, type: "number", role: "value.temperature", unit: "\xB0C" },
      { id: "data.dhw.dhwTargetSetpoint", name: "DHW Target Setpoint", read: true, write: false, type: "number", role: "value.temperature", unit: "\xB0C" },
      ...
      { id: "data.dhw.gasCalorificValue", name: "Gas Calorific Value", read: true, write: false, type: "number", role: "value.power", unit: "kWh/m\xB3" },
      ...
      { id: "data.roomThermostat.nextSetpoint", name: "next Setpoint", read: true, write: false, role: "value.temperature", type: "number", unit: "\xB0C" },
      { id: "data.roomThermostat.currentScheduleSetPoint", name: "current Schedule SetPoint", read: false, write: false, role: "value.temperature", type: "number", unit: "\xB0C" },
    ...
    for (const state of states) {
      await this.setObjectNotExistsAsync(state.id, {
        type: "state",
        common: {
          name: await (0, import_tools._translate)(state.name, systemLang),
  ...
  async updateDevices() {
    try {
      ...
      const _zoneModeTranslate = await (0, import_tools._translate)(_zoneMode, systemLang);
